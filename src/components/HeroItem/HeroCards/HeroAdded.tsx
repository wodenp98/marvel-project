...
export default function HeroAdded({
  name,
  image,
  classHero,
  stars,
  ...
  const getHero = useAddHeroesStore((state) => state.items);
  const addHero = useAddHeroesStore((state) => state.addItem);
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    ...
    // ici faire un fetch voir si le héros existe déjà dans la userDashboard
    const res = await fetch("/api/userDatabase");
    ...
    const existantHero = await res.json();

    if (
      existantHero.some(
        (hero: any) => hero.name === data.name && hero.stars === data.stars
      )
    ) {
      toast({
